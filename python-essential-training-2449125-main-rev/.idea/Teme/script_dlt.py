...
if __name__ == '__main__':
    ...
    for log_file in files:
        for crash_string in ERRORS_TO_FIND:
            CRASH_MAPPINGS.setdefault(crash_string, {'count': 0})
            errors_in_file = find_errors_in_file(log_file, crash_string)
            if errors_in_file:
                CRASH_MAPPINGS[crash_string]['count'] += len(errors_in_file)
                CRASH_MAPPINGS[crash_string][log_file] = len(errors_in_file)
    ...
    report_summary = print_errors_summary(CRASH_MAPPINGS)
    ...
    with open(args.output_file, 'w') as logs:
        ...
        logs.write("="*120+"\n")
        logs.write("*"*120+"\n")
        for key in ERRORS_FOUND.keys():
            logs.write(key+"\n")
            logs.write("-"* 120 + "\n")
            for error in ERRORS_FOUND[key]:
                logs.write(error[1]+"\n")
